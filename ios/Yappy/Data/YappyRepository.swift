...
struct YappyRepository {
     ...
     */
    func customEmojis(_ conversationId: String) async throws -> CustomEmojisEnvelope {
    ...
    func createCustomEmoji(
        ...
        try await api.post("/conversations/\(conversationId)/emojis", .object([
            "name": .string(name),
            "mediaId": .string(mediaId),
        ]))
    }

    func deleteCustomEmoji(_ conversationId: String, emojiId: String) async throws {
        try await api.send("DELETE", "/conversations/\(conversationId)/emojis/\(emojiId)")
    }
    ...
    struct MentionSpan {
        ...
        var userId: String?
        var roleId: String?
        /// A `#channel` signpost. Notifies nobody — it is a link to a place,
        /// not a call to a person.
        ...
        let entityValue: JSONValue? = mentions.isEmpty ? nil : .array(
            mentions.map { span in
                ...
                if span.userId != nil {
                    ...
                    kind = "mention_role"
                } else if span.channelId != nil {
                    kind = "mention_channel"
                } else if span.emojiId != nil {
                    kind = "custom_emoji"
                } else {
                    kind = "mention_all"
                }
                ...
                if let channelId = span.channelId { entity["channelId"] = .string(channelId) }
                if let emojiId = span.emojiId { entity["emojiId"] = .string(emojiId) }
